...
<%@ include file="/WEB-INF/jsp/moduls/includes.jsp"%>
          ...
          <c:choose>
           <c:when test="${__theFilterForm.actionsRenderer == 1}">
            ...
            <a class="btn " href="<c:url value="${contexte}/${estatSolicitudServei.estatSolicitudServeiID}/edit"/>" onclick="" title="<fmt:message key="genapp.edit"/>">
               <i class="icon-pencil"></i>
            </a>
            </c:if>
            <c:if test="${__theFilterForm.deleteButtonVisible}">
            <a class="btn btn-danger" href="#myModal" onclick="openModal('<c:url value="${contexte}/${estatSolicitudServei.estatSolicitudServeiID}/delete"/>','show');" title="<fmt:message key="genapp.delete"/>">
      ...
      <ul class="dropdown-menu pull-right" style="min-width:35px;padding:5px 5px 0px 5px;margin:0px;font-size: 12px" >
            <c:if test="${__theFilterForm.editButtonVisible}">
            <li>
            <a class="btn  btn-small a_item" style="margin-bottom:5px;" href="<c:url value="${contexte}/${estatSolicitudServei.estatSolicitudServeiID}/edit"/>" onclick="">
            <i class="icon-pencil"></i>
            ...
            <c:if test="${__theFilterForm.deleteButtonVisible}">
            <li>
            <a class="btn btn-danger btn-small a_item" style="margin-bottom:5px;color: white;" href="#myModal" onclick="openModal('<c:url value="${contexte}/${estatSolicitudServei.estatSolicitudServeiID}/delete"/>','show');">
            <i class="icon-trash icon-white"></i>

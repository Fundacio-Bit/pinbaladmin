...
public class SolicitudFullViewOperadorController extends SolicitudOperadorController {
  ...
  public SolicitudForm getSolicitudForm(SolicitudJPA _jpa, boolean __isView,
    ...
    if (__isView) {
      ...
      solicitudForm.addAdditionalButton(new AdditionalButton(" icon-envelope", "solicitud.caid",
  ...
  public ModelAndView generarFormulariCaid(HttpServletRequest request,
    ...
    mav.addObject("organismo", "Consejería de Administraciones Públicas y Modernización » (A04027005) Dirección General de Modernización y Administración Digital");
  ...
  protected void generarFormulari(Long solicitudID, Properties prop) throws Exception, I18NException {
    ...
    File plantilla = new File(System.getProperty("org.fundaciobit.pinbaladmin.template_formulari"));
    ...
    {
        FitxerJPA fitxer = new FitxerJPA("Formulario_Ramon_Roca.pdf", outputPDF.length(), "application/pdf", "" );
        
        fitxer = (FitxerJPA)fitxerEjb.create(fitxer);

        FileSystemManager.sobreescriureFitxer(outputPDF, fitxer.getFitxerID());
   
        Document doc = documentEjb.create("Formulario_Ramon_Roca (PDF)", fitxer.getFitxerID(), null, null);
    ...
    {
        FitxerJPA fitxer = new FitxerJPA("Formulario_Ramon_Roca.odt", outputODT.length(), "application/vnd.oasis.opendocument.text", "" );
        
        fitxer = (FitxerJPA)fitxerEjb.create(fitxer);
        
        FileSystemManager.sobreescriureFitxer(outputODT, fitxer.getFitxerID());
        
        Document doc = documentEjb.create("Formulario_Ramon_Roca (ODT)", fitxer.getFitxerID(), null, null);
  ...
  protected void generarServeis(Long soliID, Properties prop) throws I18NException {
    int x = 1;
...
//      }
      ...
      java.lang.String enllazNormaLegal =  prop.getProperty("FORMULARIO.DATOS_SOLICITUD.LELSERVICIOS.ID" + x + ".ENLACENOR"); 
      
      
      java.lang.String articles =  prop.getProperty("FORMULARIO.DATOS_SOLICITUD.LELSERVICIOS.ID" + x + ".ARTICULOS"); 
      java.lang.String tipusConsentiment =  prop.getProperty("FORMULARIO.DATOS_SOLICITUD.LELSERVICIOS.ID" + x + ".LDECONSENTIMIENTO"); 
      java.lang.String consentiment =  prop.getProperty("FORMULARIO.DATOS_SOLICITUD.LELSERVICIOS.ID" + x + ".CONSENTIMIENTO"); 
      java.lang.String enllazConsentiment =  prop.getProperty("FORMULARIO.DATOS_SOLICITUD.LELSERVICIOS.ID" + x + ".ENLACECON"); 
      
      SolicitudServeiJPA ss = new SolicitudServeiJPA(soliID, serveiID, estatSolicitudServeiID, normaLegal,
          enllazNormaLegal, articles, tipusConsentiment, consentiment, enllazConsentiment, notes);

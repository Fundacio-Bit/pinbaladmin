...
public class ExportFullSolicitudServeiInfoOperatorController extends SolicitudServeiController {
  ...
  public SolicitudServeiFilterForm getSolicitudServeiFilterForm(Integer pagina,
    ...
    if (solicitudServeiFilterForm.isNou()) {
      ...
      solicitudServeiFilterForm.setDefaultOrderBy(new OrderBy[] { new OrderBy(
          new SolicitudServeiQueryPath().SOLICITUD().DATAINICI().javaName, OrderType.DESC) });
  ...
  public Where getAdditionalCondition(HttpServletRequest request) throws I18NException {
    ...
    List<Solicitud> list = (List<Solicitud>) request.getSession().getAttribute(
        SOLI_LIST_ATTRIBUTE);
  ...
  public void exportFull(@PathVariable("dataExporterID") String dataExporterID,
      HttpServletRequest request, HttpServletResponse response) throws Exception,
      I18NException {

    SolicitudServeiFilterForm filterForm = getSolicitudServeiFilterForm(1, new ModelAndView(),
        request);
  ...
  public void postList(HttpServletRequest request, ModelAndView mav,
    ...
    mapEntitatLocal = (Map<Long, String>) filterForm.getAdditionalField(SOLI_ENTITAT_LOCAL)
        .getValueMap();
    mapEntitatLocal.clear();

    Map<Long, String> mapEntitatEstatal;
    mapEntitatEstatal = (Map<Long, String>) filterForm
        .getAdditionalField(SOLI_ENTITAT_ESTATAL).getValueMap();
    ...
    mapSoliServEstat = (Map<Long, String>) filterForm.getAdditionalField(SOLI_SERV_ESTAT)
        .getValueMap();
    mapSoliServEstat.clear();

    List<Solicitud> listSoli = (List<Solicitud>) request.getSession().getAttribute(
        SOLI_LIST_ATTRIBUTE);
    ...
    for (SolicitudServei solicitudServei : list) {
      ...
      // SOLICITUD-SERVEI
      mapSoliServEstat.put(id, estatSolicitudServeiEjb.executeQueryOne(
          EstatSolicitudServeiFields.NOM, EstatSolicitudServeiFields.ESTATSOLICITUDSERVEIID

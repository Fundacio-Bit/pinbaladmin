...
public class EntitatOperadorController extends  EntitatController {
  ...
  protected DocumentEntitatLocal documentEntitatEjb;
  ...
  public EntitatFilterForm getEntitatFilterForm(Integer pagina, ModelAndView mav,
    ...
    */
    ...
    if (entitatFilterForm.isNou()) {
      ...
      entitatFilterForm.addAdditionalButtonForEachItem(new AdditionalButton(
          "icon-th-list", "solicitud.llistat",
      ...
      entitatFilterForm.addAdditionalButtonForEachItem( 
          new AdditionalButton("icon-file", "documentsentitatlocal",
          "/operador/entitat/documents/{0}", "btn-success"));
  ...
  public void postList(HttpServletRequest request, ModelAndView mav,
      EntitatFilterForm filterForm,  List<Entitat> list) throws I18NException {
    ...
    */
    ...
    for (Entitat entitat : list) {
      ...
      long count = documentEntitatEjb.count(
          DocumentEntitatFields.ENTITATID.equal(entitat.getEntitatID()));
      ...
      smskv = new SelectMultipleStringKeyValue(SolicitudFields.SOLICITUDID.select, "*", SolicitudFields.PROCEDIMENTCODI.select, SolicitudFields.PROCEDIMENTNOM.select);
      ...
      List<StringKeyValue> id_nom_Solis = solicitudEjb.executeQuery(smskv,sqp.DEPARTAMENT().AREA().ENTITATID().equal(entitat.getEntitatID()));
      //log.info("          - solis NOMS: ]" + Arrays.toString(id_nom_Solis.toArray())  + "["); 
       ...
       if (id_nom_Solis.size() != 0) {
         ...
         for (StringKeyValue skv : id_nom_Solis) {
            ...
            valuediv = valuediv 
                + "<li><a href=\"" + request.getContextPath()+ "/operador/solicitudlocal/" + skv.key + "/edit\" >" 
                + code + ": " + nom + "</a></li>";
            
         }  
         valuediv += "</ul>";
         //value =  "<a href=\"#modal_infosoli_" + entitat.getEntitatID() + "\" role=\"button\" class=\"btn btn-mini\" data-toggle=\"modal\">" + llistat + "</a>";
  ...
  public ModelAndView veureDocumentCedentGet(@PathVariable("entitatLocalID") java.lang.Long entitatLocalID,
    ...
    request.getSession().setAttribute(DocumentEntitatLocalOperatorController.ENTITATLOCALID_SESSION_PROPERTY, entitatLocalID);

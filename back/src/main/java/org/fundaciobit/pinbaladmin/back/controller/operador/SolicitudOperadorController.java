...
public abstract class SolicitudOperadorController extends SolicitudController {
  ...
  public void init() {
    ...
    areaRefList.setSelects(new Select[] {        
        aqp.ENTITAT().NOM().select, AreaFields.NOM.select 
        });
    areaRefList.setSeparator(" - ");
    */

    // PER LLISTAT
    /*
    departamentRefListPerLlistat = new DepartamentRefList(this.departamentRefList);
    departamentRefList.setSelects(new Select[] {        
        DepartamentFields.NOM.select
        , new SelectConstant(" ["),
        DepartamentFields.DEPARTAMENTID.select,new SelectConstant("]") 
        });
  ...
  public void postValidate(HttpServletRequest request,SolicitudForm solicitudForm,
    ...
    if (isestatal != null) {
      ...
      if (isestatal) {
        ...
        if (entitatid == null) {
          ValidationUtils.rejectIfEmptyOrWhitespace(result, get(ENTITATLOCALID), "genapp.validation.required",
              new Object[] { I18NUtils.tradueix(ENTITATLOCALID.fullName) });
          return;
        } 
        ...
        */
        ...
        if (areaID == null) {
          ValidationUtils.rejectIfEmptyOrWhitespace(result, get(AREAID), "genapp.validation.required",
              new Object[] { I18NUtils.tradueix(AREAID.fullName) });
          return;
        }
        ...
        long entitatOfArea = areaEjb.executeQueryOne(AreaFields.ENTITATID,
         ...
         if (entitatid != entitatOfArea) {
           result.rejectValue(get(AREAID), "error.areaincorrecta",
               "L´area seleccionada no forma part de l´entitat elegida");
           return;
         }
          */
         // ===== DEPARTAMENT
         ...
         if (departamentID == null) {
          ...
          /*
          log.info(" XYZ ZZZ postValidate => departamentID = " + departamentID);

          long areaOfDepartament = departamentEjb.executeQueryOne(DepartamentFields.AREAID,
              DepartamentFields.DEPARTAMENTID.equal(departamentID));
          
          log.info(" XYZ ZZZ postValidate => areaOfDepartament = " + areaOfDepartament);
...
*/
  ...
  public SolicitudFilterForm getSolicitudFilterForm(Integer pagina, ModelAndView mav,
      ...
      /*
        ...
        solicitudFilterForm.addHiddenField(TICKETASSOCIAT);
        ...
        */
        ...
        solicitudFilterForm.addAdditionalButtonForEachItem(new AdditionalButton(
            "icon-th-list", "servei.servei.plural",
            "javascript:$('#modal_infoservei_{0}').modal('show');",
            "btn-info"));

        solicitudFilterForm.addAdditionalButtonForEachItem(new AdditionalButton(
            "icon-eye-open", "solicitud.vistacompleta", "/operador/solicitudfullview/view/{0}",
            "btn-info"));

        solicitudFilterForm.addAdditionalButton(new AdditionalButton(
            "icon-file", "exportacio.soli_servei", getContextWeb() +  "/fullexport",
            "btn-info"));
        ...
        if(estatal != null && estatal) {
          if (solicitudFilterForm.getFilterByFields() == null) {
            solicitudFilterForm.setFilterByFields(new ArrayList<Field<?>>(solicitudFilterForm.getDefaultFilterByFields()));
          }
      ...
      List<EstatSolicitud> estatsSolicitud = estatSolicitudEjb.select(new OrderBy(EstatSolicitudFields.ESTATSOLICITUDID));
  ...
  public ModelAndView exportFull(HttpServletRequest request, HttpServletResponse response,
    ...
    String url = ExportFullSolicitudServeiInfoOperatorController.CONTEXT_WEB + "/exportFull/org_fundaciobit_plugins_exportdata_ods_ODSPlugin";
    
    
    request.getSession().setAttribute(ExportFullSolicitudServeiInfoOperatorController.SOLI_LIST_ATTRIBUTE,
        list);
    ...
    __mapping.put(ENTITATLOCALID, filterForm.getMapOfEntitatForEntitatLocalID());
  ...
  public void postList(HttpServletRequest request, ModelAndView mav,
    ...
    for (Solicitud soli : list) {
      ...
      if (soli.getEstatID()  == -1L || soli.getProcedimentCodi().startsWith("CODI_")) { 
        ...
        filterForm.addAdditionalButtonByPK(soli.getSolicitudID(), 
            new AdditionalButton("icon-warning-sign icon-white", "solicitud.senseestat", "javascript:alert('Revisi estat o codi de procediment.')", "btn-danger"));
        
      } else {
        Long count = solicitudServeiEjb.count(
            Where.AND(   
        ...
        if (count != 0) {
          filterForm.addAdditionalButtonByPK(soli.getSolicitudID(), 
              new AdditionalButton("icon-warning-sign", "solicitudservei.senseestat", "javascript:alert('Revisi els estats dels serveis associats.')", "btn-danger"));
          error=true;
        }
      }
      ...
      if (map != null) {
        ...
        String img;
        img = "<img src=\"" + request.getContextPath()+ "/img/" + tipus + "_petit_on.gif\" alt=\"" + tipus+ "\" width=\"17\" height=\"14\" border=\"0\" />";
    ...
    //Map<Long, String> map;
    //map = (Map<Long, String>)filterForm.getAdditionalField(SERVEIS).getValueMap(); 
    //map.clear();
    Long key;
    //String value;
    ...
    Map<String, String> estatsSoliServ;
    {
      ...
      smskv = new SelectMultipleStringKeyValue(EstatSolicitudServeiFields.ESTATSOLICITUDSERVEIID.select,
       ...
       List<StringKeyValue> listEstats = estatSolicitudServeiEjb.executeQuery(smskv, null, new OrderBy( EstatSolicitudServeiFields.NOM) );
    ...
    for (Solicitud solicitud : list) {
      ...
      List<SolicitudServei> solicitudServeis = solicitudServeiEjb.select(SolicitudServeiFields.SOLICITUDID.equal(solicitud.getSolicitudID()));
       ...
       if (solicitudServeis.size() != 0) {
         Map<String,String> infoServeis;
         {
           ...
           List<StringKeyValue> idsnomServeis = serveiEjb.executeQuery(smskv,ServeiFields.SERVEIID.in(serveisID)); 
           //log.info("          - serveis NOMS: ]" + Arrays.toString(idsnomServeis.toArray())  + "["); 
         ...
         for (SolicitudServei ss : solicitudServeis) {
           String skvvalue = infoServeis.get(String.valueOf(ss.getServeiID()));
            ...
            String hrefServ = "<a href=\"" + request.getContextPath()+ "/operador/servei/" + ss.getServeiID() + "/edit\" >";
            valuediv = valuediv + "<td>" + hrefServ +  code + "</a></td>\n";
            valuediv = valuediv + "<td>" + hrefServ +  nom + "</a></td>\n";
            //String hrefSoliServ = "<a href=\"" + request.getContextPath()+ "/operador/solicitudservei/" + ss.getSolicitudID() + "/edit\" >";
            valuediv = valuediv + "<td>" + estatsSoliServ.get(String.valueOf(ss.getEstatSolicitudServeiID())) + "</td>\n";
         ...
         //value =  "<a href=\"#modal_infoservei_" + solicitud.getSolicitudID() + "\" role=\"button\" class=\"btn btn-mini\" data-toggle=\"modal\">" + llistat + "</a>";
  ...
  public Map<Field<?>, GroupByItem> fillReferencesForList(SolicitudFilterForm filterForm,
      ...
      log.info("SolicitudOperadorController::GroupByValue = ]" + filterForm.getGroupValue() + "[");
      ...
      for (String va : valuesStr) {
        ...
        Long count = solicitudEjb.count(SOLICITUDID.in(
            solicitudServeiEjb.getSubQuery(SolicitudServeiFields.SOLICITUDID,
                SolicitudServeiFields.SERVEIID.in(idsServei))));
        
        values.add(new GroupByValueItem(BALEARS, 
            I18NUtils.tradueix(BALEARS.javaName + "." + va), 
            va, selected && va.equals(valGroup), count));
      }
      
     
      GroupByItem balearsGroupByItem =  new GroupByItem(BALEARS, BALEARS.javaName, BALEARS.javaName,
          selected, values );
      ...
      if (estatal != null && estatal == false) {
        ...
        /*
        GroupByItem novaAreaGroupByItem =  new GroupByItem(AREA_NOVA_ID, AREA_NOVA_ID.javaName, AREA_NOVA_ID.javaName,
            selected, new ArrayList<GroupByValueItem>());
        groupByItemsMap.put(AREA_NOVA_ID,novaAreaGroupByItem);
        


        // Field AREA NOVA
        _listSKV = this.areaRefList.getReferenceList(
            AreaFields.NOM, null);
        
        for(StringKeyValue skv : _listSKV) {
        ...
        groupByItemsMap.get(AREA_NOVA_ID).setCodeLabel(
            AreaFields._TABLE_TRANSLATION);
        fillValuesToGroupByItems(_tmp, groupByItemsMap, AREA_NOVA_ID, false);
        
        
        
        List<GroupByValueItem> lgbvi = groupByItemsMap.get(AREA_NOVA_ID).getValues();
        
        for(GroupByValueItem item : lgbvi) {
          
          log.info(" XXXXXXXXXXXXX  " +  item.getCodeLabel() + " ===> " + item.getValue());
          log.info(" XXXXXXXXXXXXX  " +  item.getCodeLabel() + " ===> " + _tmp.get(item.getValue()));
          log.info(" ");
        }
  ...
  public List<StringKeyValue> getReferenceListForEntitatNOVAID(HttpServletRequest request,
   ...
   /*
    Where _w = null;
     SubQuery<Solicitud, Long> sub = new SubQuery<Solicitud, Long>(SolicitudFields.DEPARTAMENTID.select, solicitudEjb, null);
   ...
   */
     ...
     // List<Long> areasOfAllDeps = departamentEjb.executeQuery(DepartamentFields.AREAID, where, orderBy)
   ...
   /*
     ...
     for(Departament dep : depList) {
       listSKV.add(new StringKeyValue(String.valueOf(dep.getAreaID()), "Area " + dep.getAreaID()));
     }
  ...
  public List<StringKeyValue> getReferenceListForAreaNOVAID(HttpServletRequest request,
   ...
   /*
    ...
    // _w = DepartamentFields.DEPARTAMENTID.in(sub);
     ...
     DepartamentQueryPath dqp = new DepartamentQueryPath();
     SelectMultipleStringKeyValue smkv = new SelectMultipleStringKeyValue(DepartamentFields.AREAID.select, dqp.AREA().NOM().select);
...
// }
  ...
  public Where getAdditionalCondition(HttpServletRequest request) throws I18NException {
    ...
    SolicitudFilterForm filterForm = (SolicitudFilterForm) request.getSession().getAttribute(getSessionAttributeFilterForm());
  ...
  public ThreadLocal<DepartamentRefList> referenciaDepartament = new ThreadLocal<DepartamentRefList>();
  ...
  public List<StringKeyValue> getReferenceListForDepartamentID(HttpServletRequest request,
   ...
   return super.getReferenceListForDepartamentID(request, mav, solicitudForm, where);
 }
 ...
 public List<StringKeyValue> getReferenceListForDepartamentID(HttpServletRequest request,
   ...
   return super.getReferenceListForDepartamentID(request, mav, solicitudFilterForm, list, _groupByItemsMap, where);
 }

  @Override
 public List<StringKeyValue> getReferenceListForDepartamentID(HttpServletRequest request,
      ModelAndView mav, Where where)  throws I18NException {
   return referenciaDepartament.get().getReferenceList(DepartamentFields.DEPARTAMENTID, where );
 }

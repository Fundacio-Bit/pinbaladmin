...
cmd /C mvn -DskipTests clean  -Pdesenvolupament %* install
...
if %errorlevel% EQU 0 (
	...
	IF DEFINED PINBALADMIN_DEPLOY_DIR (
	  ...
	  if exist "%PINBALADMIN_DEPLOY_DIR%\pinbaladmin_plugins.ear" ( 
		del  "%PINBALADMIN_DEPLOY_DIR%\pinbaladmin_plugins.ear"
	  )

	  xcopy /Y ear\target\pinbaladmin.ear %PINBALADMIN_DEPLOY_DIR%

...
public class SolicitudEstatalDesDeFitxersMultiplesOperadorController
    extends SolicitudEstatalOperadorController {
    ...
    try {
      ...
      for (MultipartFile multipartFile : files) {
        ...
        if ("message/rfc822".equals(mime) || ".eml".equals(extension)) {
          ...
          emi.getAttachments()
              .add(new EmailAttachmentInfo(fileName, XLSX_MIME, multipartFile.getBytes()));
        } else {
          log.info(" FORMAT 4");
  ...
  public static List<SolicitudJPA> crearSolicitudsDesDeEmail(HttpServletRequest request,
     ...
     ServeiService serveiEjb, SolicitudLogicaService solicitudLogicaEjb) throws Exception {
    ...
    for (ProcedimentInfo proc : info.getProcediments().values()) {
      ...
      for (ServeiInfo servei : serveis) {
        ...
        SolicitudServeiJPA ss = new SolicitudServeiJPA(solicitudID, serveiID,
            ...
            consentiment, enllazConsentiment, notes, caduca, fechaCaduca);

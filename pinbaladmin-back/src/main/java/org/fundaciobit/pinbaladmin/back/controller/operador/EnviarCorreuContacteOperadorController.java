...
public class EnviarCorreuContacteOperadorController extends EmailOperatorController {
    ...
    public EmailForm getEmailForm(EmailJPA _jpa, boolean __isView, HttpServletRequest request, ModelAndView mav)
        ...
        final String msg = (String) request.getSession()
                .getAttribute(AbstractEventController.SESSION_ENVIARCORREU_MISSATGE);
        emailForm.getEmail().setMessage(msg);

        final String subject = (String) request.getSession()
                .getAttribute(AbstractEventController.SESSION_ENVIARCORREU_ASSUMPTE);
    ...
    public String getRedirectWhenCreated(HttpServletRequest request, EmailForm emailForm) {
        return (String) request.getSession().getAttribute(AbstractEventController.SESSION_ENVIARCORREU_CALLBACK);
    }

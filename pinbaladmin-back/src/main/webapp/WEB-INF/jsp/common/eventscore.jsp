...
<c:if test="${showOnlyPublic==false}">
    ...
    <c:set var="urlEdit" value="javascript:document.location.href='${theurl}'"/>
    <c:if test="${not isEstatal}"> 
        <c:if test="${empty personaContacteEmail}">                       
           <c:set var="urlEdit" value="javascript:alert('La sol·licitud no te definit el correu del contacte EC');" />
        </c:if>
    </c:if>

    <button class="btn btn-sm btn-danger" onclick="${urlEdit}" style="color:white"
...
<c:if test="${event.tipus == -2 }">
 ...
 <form id ="form_${event}" style="margin: 0 0 0 0" action="<%=Configuracio.getCAIDSeleniumUrl()%>/RemoteSeleniumConsulta" target="_blank">
  ...
  <input type="hidden" id="seguimiento" name="seguimiento" value="${event.caidNumeroSeguiment}">
    ...
    <tr>
    <td>
        <input class="btn btn-sm btn-warning" type="submit" value="Seguiment">
    </td>
    <td> 
    <small>
...
<c:if test="${not empty event.fitxer}">
    ...
    <a class="btn btn-sm btn-success" target="_blank"
        ...
        title="Descarregar Fitxer"> <i class="<%=IconUtils.ICON_FILE_DOWNLOAD%>"></i>

...
<un:useConstants var="TramitIServFields" className="org.fundaciobit.pinbaladmin.model.fields.TramitIServFields"/>
        ...
        <c:if test="${!gen:contains(__theForm.hiddenFields,TramitIServFields.CONSENTIMENT)}">
        <tr id="tramitIServ_consentiment_rowid">
          <td id="tramitIServ_consentiment_columnlabelid">
            <label>
              <fmt:message key="${(empty __theForm.labels[TramitIServFields.CONSENTIMENT])?'tramitIServ.consentiment':__theForm.labels[TramitIServFields.CONSENTIMENT]}" /> &nbsp;(*)
             </label>
              <c:if test="${not empty __theForm.help[TramitIServFields.CONSENTIMENT]}">
              <i class="fas fa-info-circle" title="${__theForm.help[TramitIServFields.CONSENTIMENT]}" ></i>
              </c:if>
            </td>
          <td id="tramitIServ_consentiment_columnvalueid">
          <form:errors path="tramitIServ.consentiment" cssClass="errorField alert alert-danger" />
          <c:if test="${gen:contains(__theForm.readOnlyFields ,TramitIServFields.CONSENTIMENT)}" >
          <form:hidden path="tramitIServ.consentiment"/>
          <input type="text" readonly="true" class="form-control col-md-9-optional uneditable-input" value="${gen:findValue(__theForm.tramitIServ.consentiment,__theForm.listOfValuesForConsentiment)}"  />
          </c:if>
          <c:if test="${!gen:contains(__theForm.readOnlyFields ,TramitIServFields.CONSENTIMENT)}" >
          <c:set var="containEmptyValue"  value="false" />
          <form:select id="tramitIServ_consentiment"  onchange="if(typeof onChangeConsentiment == 'function') {  onChangeConsentiment(this); };"  cssClass="form-control col-md-9-optional" path="tramitIServ.consentiment">
        ...
        <tr id="tramitIServ_consentimentpublicat_rowid">
          ...
          <td id="tramitIServ_consentimentpublicat_columnvalueid">
          <form:errors path="tramitIServ.consentimentpublicat" cssClass="errorField alert alert-danger" />
          <c:if test="${gen:contains(__theForm.readOnlyFields ,TramitIServFields.CONSENTIMENTPUBLICAT)}" >
          <form:hidden path="tramitIServ.consentimentpublicat"/>
          <input type="text" readonly="true" class="form-control col-md-9-optional uneditable-input" value="${gen:findValue(__theForm.tramitIServ.consentimentpublicat,__theForm.listOfValuesForConsentimentpublicat)}"  />
          </c:if>
          <c:if test="${!gen:contains(__theForm.readOnlyFields ,TramitIServFields.CONSENTIMENTPUBLICAT)}" >
          <c:set var="containEmptyValue"  value="false" />
          <form:select id="tramitIServ_consentimentpublicat"  onchange="if(typeof onChangeConsentimentpublicat == 'function') {  onChangeConsentimentpublicat(this); };"  cssClass="form-control col-md-9-optional" path="tramitIServ.consentimentpublicat">
            <c:forEach items="${__theForm.listOfValuesForConsentimentpublicat}" var="tmp">
                <form:option value="${tmp.key}">${tmp.value}</form:option>
                <c:if test="${empty tmp.key}">
                  <c:set var="containEmptyValue"  value="true" />
                </c:if>
        ...
        <c:if test="${!gen:contains(__theForm.hiddenFields,TramitIServFields.URLCONSENTIMENT)}">
        <tr id="tramitIServ_urlconsentiment_rowid">
          ...
          <td id="tramitIServ_urlconsentiment_columnvalueid">
            <form:errors path="tramitIServ.urlconsentiment" cssClass="errorField alert alert-danger" />
            <form:input readonly="${ gen:contains(__theForm.readOnlyFields ,TramitIServFields.URLCONSENTIMENT)? 'true' : 'false'}" cssClass="w-100 form-control  ${gen:contains(__theForm.readOnlyFields ,TramitIServFields.URLCONSENTIMENT)? ' uneditable-input' : ''}"  style="" maxlength="240" path="tramitIServ.urlconsentiment"   />

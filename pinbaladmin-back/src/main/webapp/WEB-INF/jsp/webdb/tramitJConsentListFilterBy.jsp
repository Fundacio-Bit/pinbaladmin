...
<un:useConstants var="TramitJConsentFields" className="org.fundaciobit.pinbaladmin.model.fields.TramitJConsentFields"/>
  ...
  <div id="FilterDiv" class="wellgroupfilter formbox" style="${displayFilterDiv} margin-bottom:3px; margin-left: 1px; padding:3px;">
      ...
      <div class="input-group" style="padding-right: 4px;padding-bottom: 4px;">
        ...
        <c:if test="${gen:contains(__theFilterForm.filterByFields ,TramitJConsentFields.CONSENTID)}">
            <div class="input-group" style="padding-right: 4px;padding-bottom: 4px;">
            <%-- FILTRE NUMERO DESDE-FINS --%>
              <span class="add-on"><fmt:message key="tramitJConsent.consentid" />:</span>

              <span class="add-on">&nbsp;<fmt:message key="genapp.from" /></span>
              
              <form:input cssClass="input-append input-small" path="consentidDesde" />


              <span class="add-on">&nbsp;<fmt:message key="genapp.to" />&nbsp;</span>

              <form:input cssClass="input-append input-small search-query" path="consentidFins" />

            </div>


        </c:if>
        <c:if test="${gen:contains(__theFilterForm.filterByFields ,TramitJConsentFields.TRAMITID)}">
            <div class="input-group" style="padding-right: 4px;padding-bottom: 4px;">
            <%-- FILTRE NUMERO DESDE-FINS --%>
              <span class="add-on"><fmt:message key="tramitJConsent.tramitid" />:</span>
        ...
        <c:if test="${gen:contains(__theFilterForm.filterByFields ,TramitJConsentFields.CONSENTIMENT)}">
            <%-- FILTRE STRING --%>
            <div class="input-prepend" style="padding-right: 4px;padding-bottom: 4px;">
              <fmt:message key="tramitJConsent.consentiment" var="consentiment" />
        ...
        <c:if test="${gen:contains(__theFilterForm.filterByFields ,TramitJConsentFields.CONSENTIMENTADJUNT)}">
            <%-- FILTRE STRING --%>
            <div class="input-prepend" style="padding-right: 4px;padding-bottom: 4px;">
              <fmt:message key="tramitJConsent.consentimentadjunt" var="consentimentadjunt" />

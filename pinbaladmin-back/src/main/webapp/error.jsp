<%@page
	import="org.fundaciobit.pinbaladmin.back.security.LoginException"%><%@page
	import="org.fundaciobit.genapp.common.web.i18n.I18NUtils"%><%@page
	import="org.apache.log4j.Logger"%><%@page
	import="org.fundaciobit.genapp.common.web.HtmlUtils"%><%@ page
	import="org.springframework.context.i18n.LocaleContextHolder"%><%@ page
	import="java.util.Locale"%><%@ include
	file="/WEB-INF/jsp/moduls/includes.jsp"%><%@ page isErrorPage="true"
	language="java" contentType="text/html; charset=UTF-8"
	pageEncoding="UTF-8"%><%!protected final Logger log = Logger.getLogger(getClass());%>
<%
...
try {
	...
	log.error(" =================  ENTRA ERROR.JSP (" + idError + " - "
			+ (String) request.getAttribute("javax.servlet.error.request_uri") + request.getSession().getId()
			+ ") ===================");
	...
	if (e == null) {
		...
		if (pageContext.getErrorData() != null) {
			log.error("[[" + idError + "]] Requested getRequestURI: " + pageContext.getErrorData().getRequestURI());
			log.error("[[" + idError + "]] Requested getServletName: " + pageContext.getErrorData().getServletName());
			log.error("[[" + idError + "]] Requested getStatusCode: " + pageContext.getErrorData().getStatusCode());
		}
		int tipusError = !stipusError.isEmpty() ? new Integer(stipusError).intValue() : -1;

		switch (tipusError) {
			case 403 :
				missatgeTipusError = I18NUtils.tradueix("error.jsp.403",
						(String) request.getAttribute("javax.servlet.error.request_uri"));
				break;
			case 404 :
				missatgeTipusError = I18NUtils.tradueix("error.jsp.404",
						(String) request.getAttribute("javax.servlet.error.request_uri"));
				break;
			default :
		...
		if (runAsException || (e instanceof org.springframework.web.HttpSessionRequiredException)
...
<body>
	<div class="alert alert-danger">
		...
		<c:if test="${not empty stacktrace}">
			<div>
				<b>Error:</b>${pageContext.exception.message}&nbsp;(${requestScope['javax.servlet.forward.request_uri']})</div>
		</c:if>
		<%
		}
		%>
		<div>
			<b><%=missatgeSessioInvalida%></b>
		</div>
		<br />
		...
		<div>
			...
			<br /> <b>[[[[ MESSAGE: ${pageContext.exception.message}]]]</b> <br />
			<br />
